...
// Cache asset_prices, TWR group values, MWR IRR net cashflows, debug log.
...
export async function POST(req: Request) {
  try {
    ...
    groups.forEach((_, key) => {
      ...
      if (process.env.DEBUG_IRR === '1') logCashFlows(`Group ${key} cash flows:`, netFlows, netDates);
    ...
    if (groupValues.has('Portfolio')) {
      ...
      netDates.push(endDate);
      if (process.env.DEBUG_IRR === '1') logCashFlows('Total portfolio EXTERNAL cash flows:', netFlows, netDates);
      const irr = netFlows.length > 1 ? calculateIRR(netFlows, netDates) : NaN;
    ...
    const series = dates.map((date, i) => {
      ...
      if (benchmarks) {
        ['^GSPC', '^IXIC', 'BTC-USD'].forEach(t => {
          const seriesData = historicalData[t] || [];
          const price = seriesData.find(p => p.date === date)?.close || 0;
          const initPrice = seriesData[0]?.close || 1;
          entry[t === '^GSPC' ? 'SPX' : t === '^IXIC' ? 'IXIC' : 'BTCUSD'] = initPrice > 0 ? ((price / initPrice) - 1) * 100 : 0;
        });
      }
      return entry;
    });
