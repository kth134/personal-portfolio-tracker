          ...
          <div className={cn('flex flex-col px-4 pb-4 pt-4 sm:px-6 sm:pb-6', chartSlices.length === 1 && 'md:flex-1 md:min-h-0')}>
            <div className="flex flex-col gap-4 md:grid md:h-full md:min-h-0 md:flex-1 md:grid-cols-1 md:grid-rows-4 md:gap-4">
              ...
              <div className="dashboard-metric-tile md:h-full">
                ...
                <div className={cn('dashboard-metric-value', rebalanceNeeded ? 'value-negative' : 'value-positive')}>{rebalanceNeeded ? 'Yes' : 'No'}</div>
              </div>
            </div>
          </div>
        </details>
                      ...
                      <div className="grid grid-cols-2 gap-3 items-stretch">
                        ...
                        <div className="dashboard-mobile-subpanel dashboard-mobile-subpanel-summary grid grid-rows-[auto_repeat(3,minmax(0,1fr))] gap-2">
                          ...
                          <div className="dashboard-mobile-subpanel-cell row-start-3 row-span-2 flex h-full flex-col items-center justify-center text-center">
                            ...
                            <div className={cn('mt-2 font-semibold tabular-nums', subDrift > 0 ? 'value-positive' : (subDrift < 0 ? 'value-negative' : 'text-zinc-700'))}>{subDrift > 0 ? '+' : ''}{formatPctTenth(subDrift)}</div>
                          </div>
                        </div>
                      </div>
                    </div>
                        ...
                        <div key={`mobile-${i.asset_id}`} className="dashboard-mobile-card space-y-4">
                          <div className="grid grid-cols-2 gap-x-3 gap-y-1 min-w-0">
                            ...
                            <div className="min-w-0 break-words text-right text-xs italic leading-tight [overflow-wrap:anywhere]">
                              <span className={cn("font-semibold uppercase tracking-wide", i.action === 'buy' ? 'value-positive' : i.action === 'sell' ? 'value-negative' : 'text-zinc-500')}>
                          ...
                          <div className="mt-2 grid grid-cols-2 gap-3">
                            ...
                            <div className="p-1 text-center">
                              ...
                              <p className={cn("mt-2 text-sm font-semibold tabular-nums", i.drift_percentage > 0 ? "value-positive" : (i.drift_percentage < 0 ? "value-negative" : "text-zinc-700"))}>{i.drift_percentage > 0 ? '+' : ''}{formatPctTenth(i.drift_percentage)}</p>
                            </div>
                            <div className="p-1 text-center">
                        ...
                        <TableBody>
                          {sortedItems.map((i: any) => (
                            <TableRow key={i.asset_id} className="hover:bg-muted/5 h-16 group">
                              ...
                              <TableCell className={cn("px-3 sm:px-4 text-right tabular-nums font-bold whitespace-nowrap", i.drift_percentage > 0.1 ? "value-positive" : (i.drift_percentage < -0.1 ? "value-negative" : "text-black"))}>{i.drift_percentage > 0 ? "+" : ""}{i.drift_percentage.toFixed(1)}%</TableCell>
                              ...
                              <TableCell className="px-3 sm:px-4 text-center font-bold whitespace-nowrap">
                                {i.action === 'hold' ? (
                                  ...
                                  <span className={cn(i.action === 'buy' ? "value-positive" : "value-negative")}>{i.action.toUpperCase()}</span>
                                )}
                              </TableCell>
                  ...
                  <div className="space-y-2">
                    <div className="rounded-lg border-2 border-zinc-300 bg-zinc-100/80 p-2">
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-2 text-[10px]">
                        ...
                        <MetricChip label="Net Flow" value={formatUSDWhole(outOfBandSummary.netFlow)} valueClassName={outOfBandSummary.netFlow >= 0 ? 'value-positive' : 'value-negative'} />
                      </div>
                    </div>
                  ...
                  <div key={`mobile-plan-out-${idx}`} className="dashboard-mobile-card space-y-4">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <div className="text-right">
                        <span className={cn("block text-xs font-bold", row.action === 'buy' ? "value-positive" : "value-negative")}>{row.action.toUpperCase()}</span>
                    ...
                    <div className="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                      ...
                      <MetricChip label="Drift" value={`${row.driftPct > 0 ? '+' : ''}${formatPctTenth(row.driftPct)}`} valueClassName={row.driftPct > 0 ? 'value-positive' : 'value-negative'} />
                    </div>
                  ...
                  <div className="space-y-2">
                    <div className="rounded-lg border-2 border-zinc-300 bg-zinc-100/80 p-2">
                      ...
                      <div className="mt-2 grid grid-cols-3 gap-2 text-[10px]">
                        ...
                        <MetricChip label="Net Flow" value={formatUSDWhole(supportingSummary.netFlow)} valueClassName={supportingSummary.netFlow >= 0 ? 'value-positive' : 'value-negative'} />
                      </div>
                    </div>
