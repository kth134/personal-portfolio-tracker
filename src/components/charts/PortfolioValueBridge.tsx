      ...
      <div className="h-[320px] sm:h-full w-full min-w-0">
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={rows} margin={{ top: 16, right: 14, left: 8, bottom: isMobile ? 20 : 36 }} barCategoryGap="2%" barGap={0}>
            ...
            <Bar dataKey="stepOffset" stackId="bridge" fillOpacity={0} strokeOpacity={0} isAnimationActive={false} />
            <Bar dataKey="stepValue" stackId="bridge" radius={[6, 6, 0, 0]} isAnimationActive={false}>
              {rows.map((row) => (
                <Cell key={`step-${row.name}`} fill={row.isAnchor ? 'transparent' : row.fill} stroke={row.isAnchor ? 'transparent' : row.fill} strokeWidth={1} />
              ))}
            </Bar>
            <Bar dataKey="anchorValue" stackId="bridge" radius={[6, 6, 0, 0]} isAnimationActive={false}>
              {rows.map((row) => (
